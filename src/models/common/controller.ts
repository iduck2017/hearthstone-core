...
> {
    ...
    public bind(factory: SelectorModel): void {
        ...
        this.origin.refer.selectors?.push(factory);
        this.selectors.push(selector);
        selector.then(() => this.unbind(factory));
    }

    private unbind(factory: SelectorModel): void {
        const selectors = this.origin.refer.selectors;
        const index = selectors?.indexOf(factory);
    ...
    public set<T>(target: T | undefined) {
        ...
        if (!selector) return;
        if (!selector.options.includes(target)) selector.resolve(undefined);
        selector.resolve(target);
    }
}