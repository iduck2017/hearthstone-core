...
import { StormpikeCommandoModel } from "./index";
import { WispModel } from "../wisp";
import { sleep } from "../../../utils/sleep";
import { ManaModel } from "../../../rules/mana";

describe("stormpike-commando", () => {
    const app = new AppModel();
    const stormpikeCommando = new StormpikeCommandoModel();
    const wisp = new WispModel();
    const game = new GameModel({
        playerA: new PlayerModel({
            hero: new MageModel(),
            hand: new HandModel({ cards: [stormpikeCommando] }),
    ...
    it("check-initial-state", () => {
        ...
        expect(playerA.hand.cards).toContain(stormpikeCommando);
    });

    it("battlecry-deals-two-damage-to-enemy-minion", async () => {
        stormpikeCommando.deployer.launch();
        await sleep();
        ...
        expect(wisp.role.health.current).toBeLessThanOrEqual(0);
    });
});
