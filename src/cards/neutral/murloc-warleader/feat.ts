...
export class MurlocWarleaderFeatModel extends FeatModel {
    protected _brand: symbol = Symbol('murloc-warleader-feat-model');
    ...
    // Grant +2 Attack to every other allied Murloc as a reactive aura.
    ...
    // Grant +1 max Health to every other allied Murloc as a reactive aura.
    ...
    protected _onAllyHealthDecor(decor: RoleHealthDecor) {
        ...
        if (!targetMinion?.races.includes(RaceType.MURLOC)) return;
        decor.addBuff({
            value: 1,
            type: BuffOperatorType.AURA,
            source: this,
